/* GdkPixbuf library - PEF image loader
 *     
 * Copyright (C) 2010 Christian Waßmuth <chrwassmuth@gmx.de>
 * 
 * Based on: io-jpeg.c (GdkPixbuf library - JPEG image loader)
 * 
...
//FIXME: Works only with 10MP K100D, because of absolute file offset
...
typedef struct {
	...
	gboolean firstLoad;	//First read on buffer?
	long skipOnFirstLoad;
...
static gpointer gdk_pixbuf__pef_image_begin_load (GdkPixbufModuleSizeFunc           func0,
                                                   GdkPixbufModulePreparedFunc func1, 
                                                   GdkPixbufModuleUpdatedFunc func2,
                                                   gpointer user_data,
                                                   GError **error);
static gboolean gdk_pixbuf__pef_image_stop_load (gpointer context, GError **error);
static gboolean gdk_pixbuf__pef_image_load_increment(gpointer context,
...
static gpointer
gdk_pixbuf__pef_image_begin_load (GdkPixbufModuleSizeFunc size_func,
				   GdkPixbufModulePreparedFunc prepared_func, 
				   GdkPixbufModuleUpdatedFunc updated_func,
				   gpointer user_data,
                                   GError **error)
{
	...
	src->firstLoad=TRUE;
	src->skipOnFirstLoad=9303262; //Offset for Jpeg within PEF
 ...
 */
static gboolean
gdk_pixbuf__pef_image_stop_load (gpointer data, GError **error)
{
 ...
 */
static gboolean
gdk_pixbuf__pef_image_load_increment (gpointer data,
                                       const guchar *buf, guint size,
                                       GError **error)
{
	JpegProgContext *context = (JpegProgContext *)data;
	...
	//offset addieren
	const guchar *offset_buf;
	if (src->firstLoad)
	{
		printf("first load\n");
		if(size <= src->skipOnFirstLoad)
		{
			src->skipOnFirstLoad-=size;
			return TRUE;
		}
		else
		{
			offset_buf = buf + src->skipOnFirstLoad; // 'src->skipOnFirstLoad' must be < 'size'
			size-=src->skipOnFirstLoad;
			src->firstLoad=FALSE;
	...
	printf("size %d \n",size);
	//const guchar *offset_buf = buf + 0x08df4de;
	//buf += 9303262;
	//size+=1;

	//if(size<65535) size-=16;
	...
	if (context->src_initialized && src->skip_next) {
		...
		if (src->skip_next > size) {
			...
			bufhd = offset_buf + src->skip_next;
			src->skip_next = 0;
		}
	} else {
		printf("no skip\n");
	...
	while (TRUE) {
		...
		if (num_left > 0) {
			...
			if(src->pub.bytes_in_buffer && 
			   src->pub.next_input_byte != src->buffer)
			{
				//bytes_in_buffer ist 1 bei kaputten jpeg, sonst 2
				printf("src->pub.bytes_in_buffer = %d\n",src->pub.bytes_in_buffer);
				//Hier wird next_input_byte an den Anfang des Source-Buffers verschoben
			...
			//Neue Daten werden an das freie Ende des Source-Buffer kopiert
			...
			while (!jpeg_input_complete (cinfo)) {
				...
				if (!context->in_output) {
					if (jpeg_start_output (cinfo, cinfo->input_scan_number)) {
						printf("cinfo->input_scan_number = %d\n",cinfo->input_scan_number);
...
#else
#define MODULE_ENTRY(function) void _gdk_pixbuf__pef_ ## function
#endif

MODULE_ENTRY (fill_vtable) (GdkPixbufModule *module)
{
	//module->load = gdk_pixbuf__pef_image_load;
	module->begin_load = gdk_pixbuf__pef_image_begin_load;
	module->stop_load = gdk_pixbuf__pef_image_stop_load;
	module->load_increment = gdk_pixbuf__pef_image_load_increment;
	//module->save = gdk_pixbuf__pef_image_save;
	//module->save_to_callback = gdk_pixbuf__pef_image_save_to_callback;
}
